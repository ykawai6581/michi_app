...
export const MapView = forwardRef<MapHandle, Props>(function MapView({ selected, highlightStyle, visibility, onReady }, ref) {
  ...
  useEffect(() => { const map = mapRef.current; if (map?.isStyleLoaded() && map.getSource(SOURCE_IDS.highlight) && selected) selectFeature(map, selected, highlightStyle.animate) }, [selected, highlightStyle.animate])
