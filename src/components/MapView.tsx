...
export const MapView = forwardRef<MapHandle, Props>(function MapView({ selected, highlightStyle, visibility, onReady }, ref) {
  ...
  useEffect(() => { if (!container.current) return; const map = createMap(container.current); mapRef.current = map; map.addControl(new maplibregl.NavigationControl({ showCompass: true }), 'bottom-left'); map.on('load', () => { addDataLayers(map, sampleData); onReady() }); return () => { map.remove(); mapRef.current = null } }, [onReady])
  ...
  useEffect(() => { const map = mapRef.current; if (!map?.loaded()) return; map.setLayoutProperty('paper','visibility',visibility.base ? 'visible':'none'); const groups: [string[], boolean][] = [[[LAYER_IDS.roads],visibility.roads],[[LAYER_IDS.historicalRoads],visibility.historicalRoads],[[LAYER_IDS.places],visibility.places],[[LAYER_IDS.chomeFill,LAYER_IDS.chomeLine],visibility.chome]]; groups.forEach(([ids,on]) => ids.forEach((id) => map.setLayoutProperty(id,'visibility',on?'visible':'none'))) }, [visibility])
