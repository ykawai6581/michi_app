...
export const MapView = forwardRef<MapHandle, Props>(function MapView({ selected, highlightStyle, visibility, onReady }, ref) {
  ...
  useEffect(() => { const map = mapRef.current; if (!map?.loaded()) return; const groups: [string[], boolean][] = [[[LAYER_IDS.gsiBase],visibility.base],[[LAYER_IDS.roads],visibility.roads],[[LAYER_IDS.historicalRoads],visibility.historicalRoads],[[LAYER_IDS.places],visibility.places],[[LAYER_IDS.chomeFill,LAYER_IDS.chomeLine],visibility.chome]]; groups.forEach(([ids,on]) => ids.forEach((id) => map.setLayoutProperty(id,'visibility',on?'visible':'none'))) }, [visibility])
