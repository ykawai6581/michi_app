...
export function addDataLayers(map: maplibregl.Map, data: FeatureCollection): void {
  ...
  map.addLayer({ id: LAYER_IDS.highlightLineLabels, type: 'symbol', source: SOURCE_IDS.highlight, filter: ['==',['geometry-type'],'LineString'], layout: { 'symbol-placement':'line','symbol-spacing':380,'text-field':['get','name'],'text-size':14,'text-font':['Noto Sans Regular'],'text-keep-upright':true }, paint: { 'text-color':'#1d9a8a','text-halo-color':'#fff','text-halo-width':2 } })
  map.addLayer({ id: LAYER_IDS.highlightLabels, type: 'symbol', source: SOURCE_IDS.highlight, filter: ['in',['geometry-type'],['literal',['Point','Polygon']]], layout: { 'text-field':['get','name'],'text-size':14,'text-font':['Noto Sans Regular'],'text-offset':['case',['==',['geometry-type'],'Point'],['literal',[0,1.5]],['literal',[0,0]]],'text-anchor':['case',['==',['geometry-type'],'Point'],'top','center'] }, paint: { 'text-color':'#1d9a8a','text-halo-color':'#fff','text-halo-width':2 } })
}
