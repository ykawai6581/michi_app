...
export function addDataLayers(map: maplibregl.Map, project: ProjectData): void {
  ...
  map.addLayer({ id: LAYER_IDS.highlightLineLabels, type: 'symbol', source: SOURCE_IDS.highlightLineLabels, layout: { 'symbol-placement':'point','text-field':['get','name'],'text-size':BASE_LINE_LABEL_SIZE_LARGE,'text-font':['Noto Sans Regular'],'text-rotate':['get','bearing'],'text-rotation-alignment':'map' }, paint: { 'text-color':lineColorExpression(ROAD_LABEL_COLOR),'text-halo-color':ROAD_LABEL_HALO_COLOR,'text-halo-width':ROAD_LABEL_HALO_WIDTH } })
