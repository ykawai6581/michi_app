...
import'./style.css'
import{applyStatutoryNetworkChoice,atomIdsIntersectingBounds,canBuild,canConnect,candidatePathGeoJson,ContinuityGap,deriveManualReviewLayers,diagnosticLayerIds,DiagnosticLayerId,deletionApiPaths,deletionConfirmation,emptyDiagnosticState,emptyManualSelection,emptyRoad,excludeManualAtoms,gapReviewQueue,includeGapCandidate,initialLayerVisibility,LayerVisibility,ManualSelection,PreviewStage,previewStageAfterManualEdit,removeAt,resolveDeletableRoad,Road,SelectionBounds,statutoryNetworkChoice,toggle,toggleLayerVisibility,toggleManualAtom,uniqueAdd}from'./model'
...
type ConnectedPreviewResult={finalPreviewId:string;selected:FC;continuityGaps:FC;report:Record<string,number>;n13SourceFingerprint?:string}
...
function App(){
  ...
  const[connectReport,setConnectReport]=useState<Record<string,number>>({})
  ...
  const onFeature=useCallback((properties:object,layer?:DiagnosticLayerId)=>{
    ...
    if(layer==='continuityGaps'){
      setSelectedGapId(String((properties as Record<string,unknown>).gapId||''));setSelectedCandidateIndex(0);return
    }
  ...
  const clearGapReview=()=>{setContinuityGaps([]);setConnectReport({});setIgnoredGapIds(new Set());setSelectedGapId(undefined);setSelectedCandidateIndex(0)}
  ...
  const connectSelected=async()=>{
    ...
    try{
      ...
      for(;;){
        ...
        setLayers(current=>({...current,finalConnected:completed.finalConnected,continuityGaps:job.result.continuityGaps}));setFinalPreviewId(completed.finalPreviewId)
        setContinuityGaps(gaps);setConnectReport(job.result.report);setIgnoredGapIds(new Set());setSelectedGapId(gaps.find(gap=>gap.decision.startsWith('unresolved-'))?.gapId);setSelectedCandidateIndex(0)
  ...
  const reviewQueue=gapReviewQueue(continuityGaps,ignoredGapIds)
  const currentGap=reviewQueue.find(gap=>gap.gapId===selectedGapId)||reviewQueue[0]
  const currentCandidate=currentGap?.candidatePaths[selectedCandidateIndex]
  ...
  const focusGap=()=>{
    ...
    const marker=layers.continuityGaps?.features.filter(feature=>feature.properties?.gapId===currentGap.gapId)||[]
    const localIds=new Set([currentGap.upstreamN13AtomId,currentGap.downstreamN13AtomId,...(currentCandidate?.atomIds||[])])
  ...
  const addGapCandidate=(atomIds:string[])=>{
    setManualHistory(history=>[...history,{selection:manualSelection,regionCount:exclusionRegions.features.length}])
    setManualSelection(includeGapCandidate(manualSelection,{atomIds}));setFinalPreviewId(undefined)
    setLayers(current=>({...current,finalConnected:{type:'FeatureCollection',features:[]},continuityGaps:{type:'FeatureCollection',features:[]},candidateHighlight:{type:'FeatureCollection',features:[]}}))
    setContinuityGaps([]);setConnectReport({});setIgnoredGapIds(new Set());setSelectedGapId(undefined)
    setPreviewStage('MATCH_EDITED');setStatus('Gap candidate added. Run Connect Selected again.')
  }
