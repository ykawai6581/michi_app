...
type MatchPreviewResult={previewId:string;draftHash:string;reference:FC;referenceExcluded:FC;autoSelected:FC;unselectedShortlist:FC;allCandidates:FC;residualRejected:FC;diagnostics:FC;ownership:FC}
...
function Map({layers,visibility,fitVersion,onFeature,excludeRegionMode,exclusionRegions,onExcludeRegion}:{layers:DiagnosticLayers;visibility:LayerVisibility;fitVersion:number;onFeature:(properties:object,layer:DiagnosticLayerId)=>void;excludeRegionMode:boolean;exclusionRegions:FC;onExcludeRegion:(bounds:SelectionBounds,rectangle:GeoJSON.Feature<GeoJSON.Polygon>)=>void}){
    ...
    const synchronize=()=>{
      ...
      diagnosticLayerIds.forEach(id=>{
        ...
        if(source)source.setData(data)
          ...
          instance.addLayer({id,type:id==='ownership'?'circle':'line',source:id,layout:{visibility:mapLayerVisibility(visibilityRef.current,id)},paint:layerPaint[id]} as maplibregl.LayerSpecification)
          if(['autoSelected','unselectedShortlist','manuallyIncluded','manuallyExcluded'].includes(id)){
            instance.addLayer({id:`${id}-hit`,type:'line',source:id,paint:{'line-color':'#000','line-width':14,'line-opacity':0}} as maplibregl.LayerSpecification)
  ...
  useEffect(()=>{
    ...
    const synchronize=()=>diagnosticLayerIds.forEach(id=>{
      ...
      if(source)source.setData(data)
        ...
        instance.addSource(id,{type:'geojson',data})
        instance.addLayer({id,type:id==='ownership'?'circle':'line',source:id,layout:{visibility:mapLayerVisibility(visibility,id)},paint:layerPaint[id]} as maplibregl.LayerSpecification)
        if(['autoSelected','unselectedShortlist','manuallyIncluded','manuallyExcluded'].includes(id)){
          instance.addLayer({id:`${id}-hit`,type:'line',source:id,paint:{'line-color':'#000','line-width':14,'line-opacity':0}} as maplibregl.LayerSpecification)
          instance.on('click',`${id}-hit`,event=>{if(event.features?.[0])onFeatureRef.current(event.features[0].properties||{},id)})
        }else instance.on('click',id,event=>{if(event.features?.[0])onFeatureRef.current(event.features[0].properties||{},id)})
  ...
  useEffect(()=>{
    const instance=map.current
    if(!instance)return
    const update=()=>diagnosticLayerIds.forEach(id=>{const value=mapLayerVisibility(visibility,id,Boolean(layers[id]));if(instance.getLayer(id))instance.setLayoutProperty(id,'visibility',value);if(instance.getLayer(`${id}-hit`))instance.setLayoutProperty(`${id}-hit`,'visibility',value)})
    if(instance.loaded())update();else instance.once('load',update)
  },[layers,visibility])
...
function App(){
  ...
  const excludeRegion=useCallback((bounds:SelectionBounds,rectangle:GeoJSON.Feature<GeoJSON.Polygon>)=>{
    const shortlist={type:'FeatureCollection' as const,features:[...(layers.autoSelected?.features||[]),...(layers.unselectedShortlist?.features||[])]}
  ...
  },[layers.autoSelected,layers.unselectedShortlist,exclusionRegions.features.length,manualSelection])
  ...
  const connectSelected=async()=>{
    ...
    try{
      ...
      for(;;){
        ...
        setLayers(current=>({...current,finalConnected:job.result.selected}));setFinalPreviewId(job.result.finalPreviewId)
