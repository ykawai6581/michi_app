...
import'./style.css'
import{applyStatutoryNetworkChoice,canBuild,canConnect,diagnosticLayerIds,DiagnosticLayerId,deletionApiPaths,deletionConfirmation,emptyDiagnosticState,emptyManualSelection,emptyRoad,initialLayerVisibility,LayerVisibility,ManualSelection,PreviewStage,previewStageAfterManualEdit,removeAt,resolveDeletableRoad,Road,statutoryNetworkChoice,toggle,toggleLayerVisibility,toggleManualAtom,uniqueAdd}from'./model'
...
const layerPaint:Record<DiagnosticLayerId,Record<string,unknown>>={
  ...
  candidates:{'line-color':['case',['==',['get','manualSelection'],'include'],'#00a7c4','#da8a18'],'line-width':3,'line-opacity':.65},
  ...
  selected:{'line-color':['match',['get','manualSelection'],'exclude','#777','include','#00a7c4','#d92727'],'line-width':5,'line-opacity':['case',['==',['get','manualSelection'],'exclude'],.35,.9]},
}
...
function Map({layers,visibility,fitVersion,onFeature}:{layers:DiagnosticLayers;visibility:LayerVisibility;fitVersion:number;onFeature:(properties:object,layer:DiagnosticLayerId)=>void}){
...
function App(){
  ...
  const[manualHistory,setManualHistory]=useState<ManualSelection[]>([])
  ...
  const onFeature=useCallback((properties:object,layer?:DiagnosticLayerId)=>{
    ...
    if(!atomId)return
    setManualSelection(current=>{setManualHistory(history=>[...history,current]);return toggleManualAtom(current,atomId,Boolean((properties as Record<string,unknown>).automaticSelection))})
  ...
  const invalidatePreview=()=>{setPreviewId(undefined);setFinalPreviewId(undefined);setPreviewDraftHash(undefined);setPreviewIsCurrent(false);setPreviewStage('NO_MATCH')}
