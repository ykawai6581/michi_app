...
import{LocationsField,MapClassificationField}from'./RoadFields'
...
function Map({layers,visibility,fitVersion,focusGeoJson,focusVersion,onFeature,excludeRegionMode,exclusionRegions,onExcludeRegion,locations,onPickLocation}:{layers:DiagnosticLayers;visibility:LayerVisibility;fitVersion:number;focusGeoJson?:FC;focusVersion:number;onFeature:(properties:object,layer:DiagnosticLayerId)=>void;excludeRegionMode:boolean;exclusionRegions:FC;onExcludeRegion:(bounds:SelectionBounds,rectangle:GeoJSON.Feature<GeoJSON.Polygon>)=>void;locations:Road['locations'];onPickLocation?:(coordinate:[number,number])=>void}){
  ...
  useEffect(()=>{
    ...
    const onClick=(event:maplibregl.MapMouseEvent)=>onPickLocationRef.current?.([event.lngLat.lng,event.lngLat.lat])
    ...
    const initialize=()=>{
      ...
      instance.addSource('authored-locations',{type:'geojson',data:{type:'FeatureCollection',features:[]}})
      instance.addLayer({id:'authored-location-radius',type:'circle',source:'authored-locations',paint:{'circle-radius':['get','revealRadiusPx'],'circle-color':'transparent','circle-stroke-color':'#00a7c4','circle-stroke-width':2}})
      instance.addLayer({id:'authored-location-point',type:'circle',source:'authored-locations',paint:{'circle-radius':5,'circle-color':'#00a7c4','circle-stroke-color':'white','circle-stroke-width':2}})
    ...
    return()=>{styleReady.current=false;window.removeEventListener('keydown',onKeyDown);instance.off('click',onClick);instance.remove();map.current=null}
  },[])
  ...
  useEffect(()=>{const source=map.current?.getSource('authored-locations')as GeoJSONSource|undefined;source?.setData({type:'FeatureCollection',features:locations.map(location=>({type:'Feature',properties:location,geometry:{type:'Point',coordinates:location.coordinates}}))})},[locations])
...
function App(){
  ...
  const[projectLayers,setProjectLayers]=useState<Partial<Record<ProjectLayer|'locations',FC>>>({})
