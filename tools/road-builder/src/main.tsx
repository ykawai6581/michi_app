...
function App(){
  ...
  const clearGapReview=()=>{setContinuityChecks([]);setContinuityGaps([]);setContinuitySummary(undefined);setIgnoredGapIds(new Set());setSelectedContinuityId(undefined);setSelectedCandidateIndex(0)}
  ...
  const connectSelected=async()=>{
    ...
    try{
      ...
      for(;;){
        ...
        setContinuityChecks(checks);setContinuityGaps(gaps);setContinuitySummary(job.result.continuitySummary);setIgnoredGapIds(new Set());setSelectedContinuityId(gaps[0]?.gapId);setSelectedCandidateIndex(0)
  ...
  const selectedUnresolved=selectedCheck?.decision.startsWith('unresolved-')?selectedCheck:undefined
  ...
  const addGapCandidate=(atomIds:string[])=>{
    ...
    setLayers(current=>({...current,finalConnected:{type:'FeatureCollection',features:[]},continuityChecks:{type:'FeatureCollection',features:[]},continuityGaps:{type:'FeatureCollection',features:[]},candidateHighlight:{type:'FeatureCollection',features:[]}}))
    setContinuityChecks([]);setContinuityGaps([]);setContinuitySummary(undefined);setIgnoredGapIds(new Set());setSelectedContinuityId(undefined)
