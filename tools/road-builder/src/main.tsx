...
function App(){
  ...
  const previewMatch=async()=>{
    ...
    setPreviewProgress({status:'running',progress:0,phase:'Preparing reference'});setPreviewIsCurrent(false)
    try{
      const started=await api('/api/match/preview/start','POST',{road})
      for(;;){
        await new Promise(resolve=>setTimeout(resolve,350))
        const job=await api(`/api/jobs/${started.jobId}`) as PreviewProgress&{result?:any;error?:{message:string}}
        ...
        setPreviewIsCurrent(true);setStatus('Preview ready ✓');return
      }
    }catch(reason){setError((reason as Error).message);setStatus('Preview failed');setPreviewProgress(current=>current?{...current,status:'failed',phase:'Preview failed'}:undefined)}
  }
