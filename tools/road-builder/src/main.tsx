...
function App(){
  ...
  const deleteRoad=async()=>{
    if(!editing)return
    try{
      const referenceResult=await api(`/api/roads/${editing}/references`)
      ...
      if(!window.confirm(`Delete ${road.displayName}?\n\nThis will remove the registered road and its generated road-specific outputs. Shared N13/OSM source datasets will not be deleted.${used}`))return
      const result=await api(`/api/roads/${editing}`,'DELETE')
