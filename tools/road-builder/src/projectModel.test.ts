...
import{emptyProject,initialProjectVisibility,selectHistoricalRoute,serializeProject,toggleProjectLayer}from'./projectModel'
describe('project authoring model',()=>{
 it('uses authoring defaults',()=>expect(emptyProject()).toEqual({id:'',displayName:'',bounds:{mode:'auto',from:'modernRoads',paddingKm:3},layers:{modernRoads:[],historicalRoads:[],historicalPosts:[]}}))
 ...
 it('toggles rail and stations independently',()=>{let p=toggleProjectLayer(emptyProject(),'railways');p=toggleProjectLayer(p,'stations');expect(p.layers).toMatchObject({railways:{mode:'bbox'},stations:{mode:'bbox'}});expect(toggleProjectLayer(p,'railways').layers.railways).toBeUndefined()})
 ...
 it('models all project layers as independently visible',()=>expect(initialProjectVisibility()).toEqual({modernRoads:true,railways:true,stations:true,historicalRoads:true,historicalPosts:true}))
})
