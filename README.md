...
The app loads both generated files at startup, so the downloaded roads and stations are available in the search panel and can be highlighted on the map. OSM road segments with the same normalized name or an overlapping alias—including separate carriageway directions—are combined transitively into one search result. For illustration, regularly sampled segment geometry is collapsed along its principal route axis into a smoothed single `LineString`. This derived centerline is rendered wider, avoids drawing both carriageways separately, and lets the highlight animation travel once along the road instead of restarting on every OSM segment. Source URLs from every source segment remain attached to the derived result, but the `甲州街道` search entity intentionally exposes no aliases: potentially conflicting names and road numbers such as `新宿通り`, `麹町大通り`, `20`, and `430` are used only for internal segment grouping. To check changes without deploying, run `npm run dev`, open the local URL printed by Vite, and search for `甲州街道` or a station name such as `新宿`. Use `npm run build && npm run preview` when you want to test the production build locally.
