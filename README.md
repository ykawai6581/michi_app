...
This writes the raw review data to `public/data/modern/shinjuku-osm.geojson`, the preprocessed result to `public/data/modern/shinjuku-logical-roads.geojson`, and the lightweight browser index to `public/search/modern-shinjuku.json`. The Overpass query uses ways whose exact `name` is `甲州街道` as seeds and downloads the surrounding modern road network for topology; `20`, `430`, `新宿通り`, and `麹町大通り` are not aliases or seeds for the logical road. Every source feature retains its OSM object URL, object ID, retrieval date, highway class, ref, ODbL license, and the warning that current OSM geometry is not evidence for a historical alignment.

Road reconstruction is preprocessing-based. `scripts/process/road-strokes.mjs` splits source LineStrings into graph edges, snaps coordinates within 0.75 m, and forms COINS-style strokes only when continuations are mutual best matches with at most 45° deflection. Residual seed components are considered only within a 250 m endpoint gap. Dijkstra search may use existing graph edges up to 600 m, at most four turns, and no individual deflection above 75°. Its cost combines length, turn, class-change, and exact name/ref compatibility penalties. A path needs confidence 0.72 and must beat an alternative by 0.10; otherwise the components remain disconnected. No geometry is drawn between unconnected graph nodes.

The logical GeoJSON retains source IDs, OSM IDs, per-segment methods (`direct_source`, `stroke_continuation`, or `graph_gap_completion`), inferred path IDs, scores, confidence, and the untouched source geometry. The browser loads this static result, while stations still come from the raw collection. Search for `甲州街道`, then use the search panel's `論理道路` / `元セグメント` toggle to compare the reconstructed network path with every directly downloaded source segment. Use `npm run data:roads:shinjuku` to rerun preprocessing without downloading, and `npm run dev` or `npm run build && npm run preview` to review locally.
