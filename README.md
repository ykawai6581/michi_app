...
The app loads both generated files at startup, so the downloaded roads and stations are available in the search panel and can be highlighted on the map. To check changes without deploying, run `npm run dev`, open the local URL printed by Vite, and search for `甲州街道`, `新宿通り`, `国道20号`, or a station name such as `新宿`. Use `npm run build && npm run preview` when you want to test the production build locally.
