...
The app loads both generated files at startup, so the downloaded roads and stations are available in the search panel and can be highlighted on the map. OSM road segments with the same normalized name or an overlapping alias—including separate carriageway directions—are combined transitively into one search result. For illustration, regularly sampled segment geometry is projected along its principal route axis and collapsed into a single `LineString` using a broad moving local median followed by weighted smoothing. The broad median rejects short perpendicular or otherwise misnamed strips, while regularly spaced interpolation and smoothing prevent the remaining route from becoming choppy. This uses the downloaded vector geometry rather than sampling basemap pixels: it works with every basemap mode, adds no network requests, and does not depend on road fill colors or canvas pixel access. The derived centerline is rendered wider and lets the highlight animation travel once along the road instead of restarting on every OSM segment. Source URLs from every source segment remain attached to the derived result, but the `甲州街道` search entity intentionally exposes no aliases: potentially conflicting names and road numbers such as `新宿通り`, `麹町大通り`, `20`, and `430` are used only for internal segment grouping. To check changes without deploying, run `npm run dev`, open the local URL printed by Vite, and search for `甲州街道` or a station name such as `新宿`. Use `npm run build && npm run preview` when you want to test the production build locally.

The search panel's `補間ライン` / `元セグメント` toggle switches selected roads between the derived illustration and every original downloaded OSM line. The source view keeps the road grouped as one search result but uses normal stroke width and is intended to identify the exact segment responsible for a bad interpolation.
