#!/usr/bin/env python3
"""Normalize a supplied local Geoshape historical-jurisdiction GeoJSON file."""
import argparse
...
parser.add_argument("--input", type=Path, required=True, help="Supplied local GeoJSON FeatureCollection")
...
try: write_snapshot(args.input, args.output, prefecture=args.prefecture, prefecture_name=args.prefecture_name, snapshot_date=args.snapshot_date)
except (OSError, ValueError) as error: parser.exit(2, f"Jurisdiction preprocessing failed: {error}\n")
print(f"Wrote {args.provider} {args.prefecture} snapshot {args.snapshot_date} under {args.output}")
